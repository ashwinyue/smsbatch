...
const TableNameMessageBatchJobM = "nw_message_batch_job"

// MessageBatchJobM mapped from table <nw_message_batch_job>
type MessageBatchJobM struct {
	...
	Params      *MessageBatchJobParams     `gorm:"column:params;type:longtext;comment:MessageBatch Job 参数" json:"params"`                                          // MessageBatch Job 参数
	Results     *MessageBatchJobResults    `gorm:"column:results;type:longtext;comment:MessageBatch Job 执行结果" json:"results"`                                      // MessageBatch Job 执行结果
	Status      string                     `gorm:"column:status;type:varchar(32);not null;default:Pending;comment:Job 状态" json:"status"`                            // Job 状态
	Conditions  *MessageBatchJobConditions `gorm:"column:conditions;type:longtext;comment:MessageBatch Job 运行状态" json:"conditions"`                                // MessageBatch Job 运行状态
...
// TableName MessageBatchJobM's table name
func (*MessageBatchJobM) TableName() string {
	return TableNameMessageBatchJobM
}